...
#ifdef	MD5_MISSING
static unsigned digest_md5(unsigned char *target, const char *str)
{
	assert(target != NULL);
	assert(str != NULL);
	return 0;
...
#ifdef	SHA1_MISSING
static unsigned digest_sha1(unsigned char *target, const char *str)
{
	assert(target != NULL);
	assert(str != NULL);
...
static unsigned digest_rmd160(unsigned char *target, const char *str)
{
	assert(target != NULL);
	assert(str != NULL);
	return 0;
...
#ifdef	MD5_GENERIC
static unsigned digest_md5(unsigned char *digest, const char *str)
{
	assert(digest != NULL);
	assert(str != NULL);	
...
#ifdef	SHA1_GENERIC
static unsigned digest_sha1(unsigned char *digest, const char *str)
{
	assert(digest != NULL);
	assert(str != NULL);
...
static unsigned digest_rmd160(unsigned char *digest, const char *str)
{
	assert(digest != NULL);
	assert(str != NULL);	
...
static unsigned md5(string_t &d)
{
	const char *s = *d;
	char strbuf[33];
...
static unsigned sha1(string_t &d)
{
	const char *s = *d;
	char strbuf[41];
...
static unsigned rmd160(string_t &d)
{
	const char *s = *d;
	char strbuf[41];
	unsigned char digbuf[20];
