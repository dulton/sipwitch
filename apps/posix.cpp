...
static pid_t pidfile(const char *uid)
{
	...
	snprintf(buf, sizeof(buf), DEFAULT_VARPATH "/run/sipapps");
	if(!stat(buf, &ino) && S_ISDIR(ino.st_mode) && !access(buf, W_OK))
		snprintf(buf, sizeof(buf), DEFAULT_VARPATH "/run/sipapps/pidfile");
	else 
		snprintf(buf, sizeof(buf), "/tmp/sipapps-%s/pidfile", uid);
...
static struct passwd *getuserenv(const char *uid, const char *cfgfile)
{
	...
	if(!pwd) {
		fprintf(stderr, "*** sipapps: unkown user identity; exiting\n");
		exit(-1);
	}
	...
	if(uid) {
		...
		if(!fsys::changeDir(pwd->pw_dir)) {
			snprintf(buf, sizeof(buf), DEFAULT_VARPATH "/lib/sipapps");
		...
		snprintf(buf, sizeof(buf), "%s/.sipapps", pwd->pw_dir);
	...
	if(stat(buf, &ino) || !S_ISDIR(ino.st_mode)) {
		snprintf(buf, sizeof(buf), "/tmp/sipapps-%s", pwd->pw_name);
		fsys::createDir(buf, 0770);
	}

	snprintf(buf, sizeof(buf), "%d", pwd->pw_uid);
	setenv("IDENT", "sipapps", 1);
	setenv("UID", buf, 1);
...
static void foreground(const char *uid, const char *cfgpath, unsigned priority)
{
	...
	if(0 != (pid = pidfile(pwd->pw_name, getpid()))) {
		fprintf(stderr, "*** sipapps: already running; pid=%d\n", pid);
		exit(-1);
	}

	if(!process::attach("sipapps", pwd->pw_name)) {
...
static void background(const char *uid, const char *cfgpath, unsigned priority)
{
	...
	openlog("sipapps", LOG_CONS, LOG_DAEMON);
...
static void command(const char *uid, const char *cmd, unsigned timeout)
{
	...
	if(!process::control(uid, "%d %s", getpid(), cmd)) {
		fprintf(stderr, "*** sipapps: %s; server not responding\n", cmd);
		exit(2);
	}
	...
	if(signo == SIGUSR1)
		exit(0);
	if(signo == SIGALRM) {
		fprintf(stderr, "*** sipapps: %s; server timed out\n", cmd);
		exit(1);
	}
	fprintf(stderr, "*** sipapps: %s; request failed\n", cmd);
	exit(3);
}
#endif
...
extern "C" int main(int argc, char **argv)
{
	...
	while(NULL != *(++argv)) {
		...
		if(!stricmp(*argv, "-concurrency")) {
			cp = *(++argv);
			if(!cp) {
				fprintf(stderr, "*** sipapps: concurrency option missing\n");
				exit(-1);
			}
			concurrency = atoi(cp);
			continue;
		...
		if(!stricmp(*argv, "-l") || !stricmp(*argv, "-plugins")) {
			plugins = *(++argv);
			if(!plugins) {
				fprintf(stderr, "*** sipapps: plugins option missing\n");
				exit(-1);
			}
			continue;
		}
		...
		if(!stricmp(*argv, "check")) {
			server::utils(user);
			if(!process::control(user, *argv)) {
				fprintf(stderr, "*** sipapps: %s; server cannot be checked\n", *argv);
				exit(2);
			}
			exit(0);
		}
		...
		if(!stricmp(*argv, "state")) {
			if(!argv[1]) {
				fprintf(stderr, "*** sipapps: state: selection missing\n");
				exit(-1);
			}
			if(argv[2]) {
